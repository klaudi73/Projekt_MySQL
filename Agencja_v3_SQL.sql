...
CREATE TABLE IF NOT EXISTS `agencja`.`produkty` (
  ...
  `prod__nazwa` VARCHAR(255) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `agencja`.`produkty_zaleznosc` (
  ...
  `produkty_zaleznosc__parent__ID` INT(11) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `agencja`.`spotkania_miejsca` (
  ...
  `spotkania_miejsca__opis` BLOB NULL DEFAULT NULL,
...
-- Table `agencja`.`users`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `agencja`.`users` ;

CREATE TABLE IF NOT EXISTS `agencja`.`users` (
  `users__ID` INT(11) NOT NULL AUTO_INCREMENT,
  `users__login` VARCHAR(45) NULL DEFAULT NULL,
  `users__pass` VARCHAR(45) NULL DEFAULT NULL,
  `users__perm` VARCHAR(45) NULL DEFAULT NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `agencja`.`lista_spotkan_z_klientami` (`spotkanie__ID` INT, `spot__termin` INT, `spot__przypomnienie_dni` INT, `spot__przypomnienie_godz` INT, `spot__przypomnienie_min` INT, `spot__odbylo_sie` INT, `klient_klient__ID` INT, `klient__ID` INT, `klient__imie` INT, `klient__nazwisko` INT, `klient__pesel` INT, `klient__numertelefonu` INT, `spotkania_miejsca__ID` INT, `spotkania_miejsca__nazwa_miejsca` INT, `spotkania_miejsca__ulica` INT, `spotkania_miejsca__nr_domu` INT, `spotkania_miejsca__nr_lokalu` INT, `spotkania_miejsca__miasto` INT, `spotkania_miejsca__panstwo` INT, `spotkanie_miejsce__ID` INT, `spotkania_miejsca_spotkanie_miejsce__ID` INT, `spotkanie_miejsce__spotkanie_spotkanie__ID` INT);
...
USE `agencja`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `agencja`.`lista_spotkan_z_klientami` AS select `agencja`.`spotkanie`.`spotkanie__ID` AS `spotkanie__ID`,`agencja`.`spotkanie`.`spot__termin` AS `spot__termin`,`agencja`.`spotkanie`.`spot__przypomnienie_dni` AS `spot__przypomnienie_dni`,`agencja`.`spotkanie`.`spot__przypomnienie_godz` AS `spot__przypomnienie_godz`,`agencja`.`spotkanie`.`spot__przypomnienie_min` AS `spot__przypomnienie_min`,`agencja`.`spotkanie`.`spot__odbylo_sie` AS `spot__odbylo_sie`,`agencja`.`spotkanie`.`klient_klient__ID` AS `klient_klient__ID`,`agencja`.`klient`.`klient__ID` AS `klient__ID`,`agencja`.`klient`.`klient__imie` AS `klient__imie`,`agencja`.`klient`.`klient__nazwisko` AS `klient__nazwisko`,`agencja`.`klient`.`klient__pesel` AS `klient__pesel`,`agencja`.`klient`.`klient__numertelefonu` AS `klient__numertelefonu`,`agencja`.`spotkania_miejsca`.`spotkania_miejsca__ID` AS `spotkania_miejsca__ID`,`agencja`.`spotkania_miejsca`.`spotkania_miejsca__nazwa_miejsca` AS `spotkania_miejsca__nazwa_miejsca`,`agencja`.`spotkania_miejsca`.`spotkania_miejsca__ulica` AS `spotkania_miejsca__ulica`,`agencja`.`spotkania_miejsca`.`spotkania_miejsca__nr_domu` AS `spotkania_miejsca__nr_domu`,`agencja`.`spotkania_miejsca`.`spotkania_miejsca__nr_lokalu` AS `spotkania_miejsca__nr_lokalu`,`agencja`.`spotkania_miejsca`.`spotkania_miejsca__miasto` AS `spotkania_miejsca__miasto`,`agencja`.`spotkania_miejsca`.`spotkania_miejsca__panstwo` AS `spotkania_miejsca__panstwo`,`agencja`.`spotkanie_miejsce`.`spotkanie_miejsce__ID` AS `spotkanie_miejsce__ID`,`agencja`.`spotkanie_miejsce`.`spotkania_miejsca_spotkanie_miejsce__ID` AS `spotkania_miejsca_spotkanie_miejsce__ID`,`agencja`.`spotkanie_miejsce`.`spotkanie_spotkanie__ID` AS `spotkanie_miejsce__spotkanie_spotkanie__ID` from (((`agencja`.`spotkanie` join `agencja`.`klient`) join `agencja`.`spotkania_miejsca`) join `agencja`.`spotkanie_miejsce`) where ((`agencja`.`klient`.`klient__ID` = `agencja`.`spotkanie`.`klient_klient__ID`) and (`agencja`.`spotkanie`.`spotkanie__ID` = `agencja`.`spotkanie_miejsce`.`spotkania_miejsca_spotkanie_miejsce__ID`) and (`agencja`.`spotkanie_miejsce`.`spotkania_miejsca_spotkanie_miejsce__ID` = `agencja`.`spotkania_miejsca`.`spotkania_miejsca__ID`));
